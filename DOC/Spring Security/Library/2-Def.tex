...
Die Filterchain basiert vollständig auf dem Java Servlet Standard. Sie entscheidet ob Spring Security anwendbar ist für eine speizifsche URL oder nicht.
...
\item UsernamePasswortAuthenticationFilter, CasAuthenticationFilter, BasicAuthenticationFilter: Fängt direkte Authentication Requests ab, wie z.B. die BasicHTTPAuthentication.
\item RememberMeAuthenticationFilter: wird verwendet um einen User zu Erinnern, d.h. es wird eine Authentication generiert, ohne wirkliche Authentisierung. Dies kann nützlich sein um Userbezogenen Inhalt auf öffentlichen Seiten zu benutzen.
\item ExceptionTranslationFilter Wird benutzt falls der AuthenticatoinEntryPoint einen Fehler zurückgibt (z.B. 403 Access Denied)
\item FilterSecurityInterceptor Um URLS zu schützen und Exceptions zu werfen, falls der Access Denied ist.
\end{itemize}
